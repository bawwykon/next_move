-- 0048_trial_mastery_bonus.sql
-- Trial mastery (relaxed rule, owner-approved): when the +1000 weekly bonus
-- pays (exactly the 3rd completion of the Mon-Sun week), the server also
-- drops +60 mastery in the active trial track. The trial follows the ISO
-- week number rotation strength -> endurance -> mobility -> discipline
-- (mirrored client-side in src/domain/board/weeklyTrial.ts, which keys off
-- the same local calendar date as day_key, so both sides always agree).
-- Relaxed means the bonus keys off the weekly payout only, never the
-- quest's categories: an untouched trial track still earns its +60 via its
-- own upsert below (discipline can never miss: it accrues on every event).
-- Re-emits apply_completion_progression from 0047 verbatim except the trial
-- accrual; both complete_quest and complete_custom_workout delegate here.
-- (mastery_level_for_points 1..20 + the 'mastery' achievement kind are
-- re-emitted unchanged for self-containment.)
...
begin
  ...
  -- Trial mastery (0048, relaxed): when the weekly bonus pays, +60 lands in
  -- the ISO-week trial track. extract(week) is the ISO-8601 week of the
  -- local day_key, exactly what the client rotation keys off.
  v_trial_track := case ((extract(week from p_day)::int - 1) % 4)
    when 0 then 'strength'::text
    when 1 then 'endurance'::text
    when 2 then 'mobility'::text
    else 'discipline'::text
  end;
  v_trial_gain := case when v_weekly = 1000 then 60 else 0 end;
  ...
  -- plus the +60 trial bonus in the trial track when the weekly pays (0048).
  for v_track in
    select t.track
  ...
  loop
    ...
    v_pts := 30 + case when v_trial_gain > 0 and v_track = v_trial_track then 60 else 0 end;
    select coalesce((
    ...
    values (p_profile, v_track, v_pts)
    on conflict (profile_id, track)
    do update set points = public.mastery.points + v_pts
  ...
  v_pts := 15 + case when v_trial_gain > 0 and v_trial_track = 'discipline' then 60 else 0 end;
  select coalesce((
      select m.points from public.mastery m
       where m.profile_id = p_profile and m.track = 'discipline'), 0)
    into v_before_pts;
  insert into public.mastery (profile_id, track, points)
  values (p_profile, 'discipline', v_pts)
  on conflict (profile_id, track)
  do update set points = public.mastery.points + v_pts
  returning points into v_after_pts;
  ...
  -- Trial track untouched by this quest still earns its +60 (relaxed rule:
  -- the bonus keys off the weekly payout, never the quest's categories).
  if v_trial_gain > 0 and v_trial_track in ('strength', 'endurance', 'mobility')
     and not (p_categories @> array[v_trial_track]) then
    select coalesce((
      select m.points from public.mastery m
       where m.profile_id = p_profile and m.track = v_trial_track), 0)
      into v_before_pts;
    insert into public.mastery (profile_id, track, points)
    values (p_profile, v_trial_track, 60)
    on conflict (profile_id, track)
    do update set points = public.mastery.points + 60
    returning points into v_after_pts;
    v_mastered := array_append(v_mastered, v_trial_track);
    v_mastery := v_mastery || jsonb_build_object(
      'track', v_trial_track,
      'points_before', v_before_pts,
      'points_after', v_after_pts,
