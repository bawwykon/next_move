-- 0043_first_perfect_endless.sql
-- Owner-approved earning expansion (no base-curve change):
-- 1. First-clear +50 the first time a profile completes a quest row.
--    Custom workouts share one sentinel row, so the first custom ever pays.
-- 2. Perfect week +1500 when a completion completes all 7 distinct local
--    days of the Mon-Sun week (more than the 7x150 daily sum). Paid exactly
--    once: the day-set must be complete including this row but not without.
-- 3. Endless streak ladder: 200 -> 3500, 365 -> 6000 (~17 XP per streak-day,
--    matching the 50/150/500/1500 curve). Same unique-gate pattern.
-- Re-emits apply_completion_progression from 0039 verbatim except the above;
-- both complete_quest and complete_custom_workout delegate here. Payload xp
-- gains 'first' and 'perfect' lines (old stored payloads lack them; the
-- client treats missing lines as 0).
