-- 0052_custom_workouts_advance_journey.sql
-- Owner call: custom workouts are real quests and should count toward the
-- journey like authored ones (chapters unlock on total quest count). Until
-- now complete_custom_workout passed p_advance_journey => false, freezing
-- profiles.journey_quests / chapter on custom completions while every other
-- system (XP, level, streak, daily/weekly, mastery, achievements) ran.
-- Re-emit complete_custom_workout exactly as 0042 specifies, changing only
-- the flag to true. Client needs no change: journey screens read
-- profiles.journey_quests / current_chapter, which now advance.
-- First-clear stays once-per-account (all customs share the
-- 'custom-workout' sentinel quest_id), unchanged.
