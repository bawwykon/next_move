-- 0055_custom_workout_supported_hard.sql
-- Owner correction to the 0054 score tiers: 4 normals without hard must stay
-- normal (volume hard needs 5+ normals, as before). Hard = supported hard
-- (1+ hard blocks AND 4+ points, e.g. 1 hard + 2 normal) or sheer normal
-- volume (5+ normal blocks with 0 hard). Normal = 2+ points, else easy.
-- Re-emit complete_custom_workout exactly as 0054 specifies otherwise
...
begin
  ...
  -- Hard (400 XP): 1+ hard blocks with 4+ points, or 5+ normal blocks with
  -- 0 hard. Normal (200 XP): 2+ points. Else Easy (100 XP).
  if (v_hard_count >= 1 and v_hard_count * 2 + v_normal_count >= 4)
     or (v_hard_count = 0 and v_normal_count >= 5) then
    v_base_xp := 400;
