-- 0042_custom_workout_raise_caps_16_1200.sql
-- Owner request: 12 blocks / 15 min cap is too tight (a 12-block build
-- with rests lands ~9 min and blocks further adds). Raise to 16 blocks /
-- 20 min (1200s). Floor stays 480s. Client mirrors in
-- src/domain/customWorkout/model.ts (MAX_SEGMENTS/MAX_TOTAL_SEC) carry
-- the same values.
