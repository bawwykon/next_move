...
-- The curve helpers (mastery_level_for_points etc.) are unchanged and stay as
-- migrated in 0020; the client mirror mirrors curves only, so it is untouched.
...
begin
  ...
  select coalesce((
    ...
    into v_before_pts;
  insert into public.mastery (profile_id, track, points)
  values (v_profile, 'discipline', 15)
  on conflict (profile_id, track)
  ...
  -- enter the payload. Replay path is untouched â€” a stored payload returns
  ...
  -- Step 9 - payload (Ref 05 Â§#dim) and commit to the stored completion row.
