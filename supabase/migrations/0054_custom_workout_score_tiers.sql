-- 0054_custom_workout_score_tiers.sql
-- Owner call: tier by density score, not by hard seconds. A hard block is
-- worth 2 points, a normal (intermediate) block 1 point; easy/beginner and
-- rest blocks score nothing. 4+ points = hard (1 hard + 2 normal; 4 hard;
-- 5 normal …), 2-3 = normal, 0-1 = easy. This replaces the 45s-hard-time
-- gate: a lone hard block diluted in easy filler scores 2 = normal, while
-- supported hard (backed by 2+ normal blocks) scores 4 = hard.
-- Intensity and duration stay separate concerns: the 480s floor guarantees a
-- real session, the score guarantees honest intensity.
-- Re-emit complete_custom_workout exactly as 0052 specifies otherwise
-- (16-block cap, 480-1200s window, 30/45/60s blocks, 30s rests, timer gate,
-- p_advance_journey => true so customs count toward the journey).
...
begin
  ...
  if v_seg_count < 1 or v_seg_count > 16 then
  ...
  if v_total_sec < 480 or v_total_sec > 1200 then
  ...
  -- Tier scoring (owner rule): hard block = 2 pts, normal = 1, easy/rest = 0.
  -- Hard (400 XP): score >= 4. Normal (200 XP): score >= 2. Else Easy (100 XP).
  if v_hard_count * 2 + v_normal_count >= 4 then
    v_base_xp := 400;
  elsif v_hard_count * 2 + v_normal_count >= 2 then
    v_base_xp := 200;
  else
    v_base_xp := 100;
  end if;
